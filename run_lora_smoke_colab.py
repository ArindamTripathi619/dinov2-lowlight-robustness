...
def low_light(image, severity, rng=None):
    """Apply low-light degradation. `rng` threads a seeded generator through;
    when None, NumPy's global state is used (fine for single-threaded eval)."""
...
class AugmentedCIFAR10(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if rng.random() < 0.7:
            ...
            img = low_light(img, severity, rng=rng)
        if rng.random() < 0.5:
